...
namespace CommonLibraryP.MachinePKG
{
	public class ModbusTCPMachine : Machine
	{
		...
		private ushort parallelAmount = 2;
			...
			foreach (var tagsByStation in modbusTagsByStation)
			{
				...
				foreach (var stationAndTags in updateBytimeTags)
				{
					...
					if (boolInputTags is not null && boolInputTags.Count > 0)
					{
						...
						//var discreteinputtasks = new List<Task<bool[]>>();
						//var discreteinputoffsets = new List<ushort>();
						...
						//                      var discreteinputtask = master!.ReadInputsAsync(stationAndTags.station, (ushort)(boolInputStart + boolInputOffset), (ushort)batchSize);
						//                      discreteinputtasks.Add(discreteinputtask);
						//	discreteinputoffsets.Add(boolInputOffset);
						...
						//	if (discreteinputtasks.Count >= parallelAmount)
						//	{
						//		var results = await Task.WhenAll(discreteinputtasks);
						//		for (int i = 0; i < results.Length; i++)
						//		{
						//			var partial = results[i];
						//			if (partial != null)
						//			{
						//				Array.Copy(partial, 0, boolInputResult, discreteinputoffsets[i], partial.Length);
						//			}
						//		}
						//		discreteinputtasks.Clear();
						//		discreteinputoffsets.Clear();
						//	}
						//}
						...
						var boolInputResult = await ReadInParallel((start, size) => master!.ReadInputsAsync(stationAndTags.station, start, size), boolInputStart, boolInputAmount, coilBatch, parallelAmount);
					...
					if (boolOutputTags is not null && boolOutputTags.Count > 0)
					{
						...
						//var coilstasks = new List<Task<bool[]>>();
						//var coilsoffsets = new List<ushort>();
						...
						//                  }
						var boolOutputResult = await ReadInParallel(
							  (start, size) => master!.ReadCoilsAsync(stationAndTags.station, start, size),
							  boolOutputStart,
							  boolOutputAmount,
							  coilBatch,
							  parallelAmount
					...
					if (ushortOrStringInputTags is not null && ushortOrStringInputTags.Count > 0)
					{
						...
						var ushortOrStringInputResult = await ReadInParallel(
						(start, size) => master.ReadInputRegistersAsync(stationAndTags.station, start, size),
						ushortOrStringInputStart,
						ushortOrStringInputAmount,
						registerBatch,
						parallelAmount
		);
					...
					if (ushortOrStringOutputTags is not null && ushortOrStringOutputTags.Count > 0)
					{
						...
						var ushortOrStringOutputResult = await ReadInParallel(
							(start, size) => master.ReadHoldingRegistersAsync(stationAndTags.station, start, size),
							ushortOrStringOutputStart,
							ushortOrStringOutputAmount,
							registerBatch,
							parallelAmount
		);
		...
		private async Task<T[]> ReadInParallel<T>(Func<ushort, ushort, Task<T[]>> readFunc, ushort startIndex, ushort totalAmount, ushort batchSize, int parallelAmount)
		{
			T[] result = new T[totalAmount];
			...
			while (remaining > 0)
			{
				...
				// 一次收集 parallelAmount 個 batch
				for (int i = 0; i < parallelAmount && remaining > 0; i++)
				{
					ushort size = Math.Min(remaining, batchSize);
					var task = readFunc((ushort)(startIndex + offset), size);
					tasks.Add(task);
					offsets.Add(offset);
